...
| # | 유형 | 제목 | 핵심 주제 |
|---|------|------|-----------|
| L01 | 강의 | 월드모델의 정의와 기원 | 작업 인터페이스, 렌더링/시뮬레이션/계획, Craik, 예측 부호화, 역량 분류 |
| L02 | 강의 | 상태 추정, 관측 인코딩과 잠재 동역학 | 부분 관측성, 믿음 상태, VAE, GRU → MDN-RNN → RSSM, 자유 롤아웃 |
...
| 단계 | 먼저 읽기 | 그다음 실습 |
| --- | --- | --- |
| 기초 | L01 | 역사와 분류에 앞서 공통 인터페이스 용어 구축 |
| 상태 추정 | L02: 관측, 상태, 믿음 상태 | 부분 관측과 이력을 잠재 상태에 연결 |
| 표현 | L02: 관측 인코딩 | P01: VAE 인코더 학습 |
| 동역학과 롤아웃 신뢰성 | L02: 잠재 동역학, 학습 분포와 자유 롤아웃 | P02: RSSM 동역학 모델 구축 및 진단 |
...
먼저 L01을 읽고, L02의 관측/상태/믿음 상태와 관측 인코딩 뒤 P01을 완료합니다. 이어서 잠재 동역학과 자유 롤아웃을 읽고 P02를 진행합니다. L03은 P03, P04와 함께 익히고, 선택형 최신 연구를 거쳐 L04와 P05/P06을 적용한 뒤 L05로 마무리합니다.
