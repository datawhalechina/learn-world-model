...
A trilingual VitePress curriculum teaching world models through paired lectures and hands-on projects, for readers with fundamental deep learning and RL knowledge. Chinese (site root/default locale), English, and Korean are parallel editions: keep them structurally and semantically aligned unless the user asks otherwise.
...
- `docs/` — VitePress site. `docs/.vitepress/config.mts` defines the per-locale navigation/sidebars (`root` = zh, plus `en` and `ko`).
- `docs/zh/`, `docs/en/`, `docs/ko/` — parallel editions. `docs/*/lectures/`, `docs/*/projects/`.
- `scripts/build-notebook-pages.ts` — regenerates project Markdown from notebooks.
- `scripts/dev-docs.ts` — runs the notebook watcher and `vitepress dev` together; backs `npm run docs:dev`.
- `README.md`, `README-CN.md`, `README-KO.md` — root-level readmes per language; keep their cross-links and content in sync when one changes.

## Editing Invariants

- Read the surrounding section first; match its depth, terminology, tone.
- Mirror structural and prose changes across ZH/EN/KO unless told otherwise.
...
- Split long pages at a clear conceptual boundary and update all three sidebars.
...
Also verify: every EN lecture page has ZH and KO counterparts; every sidebar target resolves; relative links resolve; lecture indexes/roadmap/homepage/project prerequisites describe the same order; generated project Markdown wasn't edited directly.
