...
| # | 项目 | 前置 | 产出 | 交付物 |
|---|------|------|------|--------|
| P01 | [训练 VAE 编码器](./p01_vae_encoder) | L02：观察、状态与信念；观测编码 | `vae_encoder.pt` | 64×64 帧上的 CNN VAE。ELBO 损失曲线。用潜在遍历检查各维度学到的因素 |
| P02 | [构建 RSSM 动力学模型](./p02_rssm_dynamics) | P01、L02：潜在动力学；训练分布与自由 rollout | `rssm.pt` | GRU、MDN-RNN 与 RSSM 对比。teacher-forced 与自由 rollout。1 步到 5 步预测误差曲线 |
| P03 | [训练 Dreamer 智能体](./p03_dreamer_agent) | P02、L03：规划与控制 | `dreamer.pt` | 编码器 + RSSM + 潜在 Actor-Critic 训练循环。奖励曲线。FID 与奖励相关性自评 |
| P04 | [替换动力学骨干网络](./p04_transformer_backbone) | P03、L03：骨干选择 | `transformer_wm.pt` | 用 STORM 风格的类别 VAE 加因果 Transformer 替换 RSSM。架构对比报告 |
