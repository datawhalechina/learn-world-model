...
description: 九条架构路线各自押注的核心假设，CWM 特例，以及带着三个问题往下走。
lecture: 5
---
...
L03 已经从工程选型角度对比了九大架构族的优劣。这里换一个视角：每条路线背后押注的是什么假设？这个假设成立，意味着什么？
...
| RWM | 机器人领域真正的瓶颈是长程 rollout 稳定性和抗 policy exploitation 能力，而非单纯的生成质量 | 具备不确定性感知和 self-forcing 训练的世界模型会比更大的骨干网络更早实现真实部署 |
| 空间 3D/4D | 物体恒存性和多视角一致性需要显式 3D 几何，仅靠足够大的扁平潜向量做不到 | 场景级 3D 表示将成为具身智能和自动驾驶世界模型的默认状态形式 |
| Genie | 动作可以从无标注视频中被发现，而不需要成对的动作标签 | 互联网规模的视频（而非遥操作数据）将成为交互式世界模型的主要数据来源 |
| LoopWM | 动力学模型的有效深度可以通过循环、谱稳定化的计算与参数量解耦 | 迭代式潜在深度会成为与模型规模、数据量并列的第三条 scaling 轴 |
...
这些赌注不是互斥的，但它们对"智能的核心是什么"的回答彼此冲突。RNN 派认为状态表示是核心，Transformer 派认为序列建模是核心，Diffusion 派认为生成忠实度是核心，JEPA 派认为语义抽象是核心，RWM 派认为部署可靠性是核心，空间 3D/4D 派认为显式几何是核心，Genie 派认为从无标注视频中发现动作是核心，LoopWM 派认为迭代深度是核心，WAM 派认为动作和感知的联合建模才是核心，而 CWM 则提出了一个横跨语言派与世界模型派的问题：如果"世界"是一个 Python 解释器，LLM 学会在里面做预测，它算哪边？
...
CWM 是用于比较的领域扩展，不是第三讲九个动力学骨干之外的”第十种架构”。九架构综述讨论用于物理或视觉预测的模型结构，而 CWM 追问的是：当被建模的环境变成程序解释器时，同一套能力标准是否仍然成立。
