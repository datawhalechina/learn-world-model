...
**先预告几个反复出现的名字。** 本分类体系会用几个系统作为反复出现的例子，课程后面才会真正从零构建它们。**JEPA**（Joint Embedding Predictive Architecture）在语义空间里预测未来的表征，而不是预测像素（完整机制见第三讲）。**Dreamer** 及其动力学核心 **RSSM**（Recurrent State Space Model）把观测压缩成潜在状态，再在潜在空间里向前预测（第二讲会亲手搭建，第三讲会与其他架构对比）。**MuZero** 完全不重建像素，直接从内部状态预测奖励和价值，并用树搜索来规划（第三讲 Part A 详述）。**TD-MPC** 把学到的 Q 函数和基于搜索的规划结合起来（同样在第三讲 Part A）。这几个名字现在都不需要深入理解，这里只关心它们各自达到了哪个能力层级。
...
L1-L5 阶梯描述系统能做什么，却还没有解释编码器、动力学模型、规划器和数据管线应如何连接成一个可部署的智能体。这个区别很重要，但要把它讲清楚，读者需要先掌握第三讲介绍的规划机制。

此时只需记住一个观念：更高层级的能力通常来自预测模型及其使用方式的组合。学完 CEM-MPC、潜在 Actor-Critic 和 TD-MPC 后，再阅读[世界模型接入完整系统的七种模式](../lecture-03-architecture-patterns/07-system-integration-patterns)，比较预测可以进入系统的哪些位置：表征预训练、训练信号、动作选择或数据生成。
