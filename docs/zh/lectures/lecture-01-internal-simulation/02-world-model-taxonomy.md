...
> **📖 先预告几个反复出现的名字**：本分类体系会用几个系统作为反复出现的例子，课程后面才会真正从零构建它们。**JEPA**（Joint Embedding Predictive Architecture）在语义空间里预测未来的表征，而不是预测像素（完整机制见第三讲）。**Dreamer** 及其动力学核心 **RSSM**（Recurrent State Space Model）把观测压缩成潜在状态，再在潜在空间里向前预测（第二讲会亲手搭建，第三讲会与其他架构对比）。**MuZero** 完全不重建像素，直接从内部状态预测奖励和价值，并用树搜索来规划（第三讲 Part B 详述）。**TD-MPC** 把学到的 Q 函数和基于搜索的规划结合起来（同样在第三讲 Part B）。这几个名字现在都不需要深入理解，这里只关心它们各自达到了哪个能力层级。
