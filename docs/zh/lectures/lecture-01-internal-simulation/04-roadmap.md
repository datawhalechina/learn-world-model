...
| 阶段 | 先阅读 | 再实践 | 完成后应该能解释 |
| --- | --- | --- | --- |
| 状态估计与表示 | L01，然后读 L02“观察、状态与信念”和“观测编码” | [P01：训练 VAE 编码器](../../projects/p01_vae_encoder) | 观察为何不等于状态，编码器保留和舍弃了什么信息 |
| 动力学与 rollout | L02“潜在动力学”“训练分布与自由 rollout”及 Dreamer 系列 | [P02：构建 RSSM](../../projects/p02_rssm_dynamics) | 有用的潜在状态为何需要记忆与不确定性，单步准确为何不能保证长时程稳定 |
| 控制 | L03“规划与控制” | [P03：训练 Dreamer 智能体](../../projects/p03_dreamer_agent) | 想象轨迹如何训练 actor 与 critic |
| 替代架构 | L03“骨干选择” | [P04：替换动力学骨干](../../projects/p04_transformer_backbone) | 何种瓶颈足以支持我们替换 RSSM |
...
L02 先问一个比压缩更早的问题：**一张 64×64 图像没有展示速度和遮挡信息时，模型怎样估计当前状态？** 建立观察、状态与信念的区别后，再用变分自编码器（VAE）学习紧凑表示。

读完状态估计与编码部分就完成 P01，不要等到 L02 全部结束。随后回到动力学和自由 rollout 部分，把学到的表示接入 RSSM，再完成 P02。到这里，你会亲手写出本课程最重要的两个预测组件，并观察单步误差怎样在模型自己的输入分布中随时程变化。
