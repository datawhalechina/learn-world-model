---
title: 潜在动力学与规划效率：TD-MPC
description: 针对潜在 MPC 架构的一致性损失、表示崩塌诊断、latent space 可视化与规划效率评估。
lecture: 4
---

# 潜在动力学与规划效率：以 TD-MPC 为例

## TD-MPC（潜在 MPC）

*TD-MPC 在 L03 中是概念对照，并不是 P04 的实现内容。本页用它说明一类可迁移的诊断：只要规划依赖隐式潜在转移模型，就需要检查潜在一致性与规划效率。*
...
**诊断规则（表示崩塌）**：如果去掉 `sg` 后一致性损失反而更低，说明编码器已退化为恒等映射，把所有状态压缩到同一个点。正式诊断：在验证集上检查潜在向量的协方差矩阵秩，秩接近 1 即为崩塌（`sg` 机制详见 L03“规划与控制”的 TD-MPC 一节）。
