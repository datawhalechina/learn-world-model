...
小球位于桌面中央的例子贯穿了前两页：单帧无法确定速度，压缩这一帧的编码器也不保证保留速度。有了 VAE 编码器，我们能把当前帧 $\mathbf{o}_t$ 压缩为 $\mathbf{z}_t$，还需要用历史补足隐藏变量并**预测未来**：
...
GRU 的优点是简单、训练稳定。缺点是输出确定性预测，无法表达**不确定性**。真实环境中，同一动作可能导致多种不同结果：推一下前面几页里的小球，它可能顺畅地滚出去，也可能撞上画面外的障碍物而停住，一个确定性预测无法同时表达这两种可能。
...
**RSSM（Recurrent State Space Model，循环状态空间模型）** 由 PlaNet 引入，随后成为 Dreamer 系列的核心状态模型。它将状态分为两部分：
...
**手算一遍（简化版单步转移）**：真实的 $f_\phi$ 是一个带门控的 GRU，不便手算，这里用一个去掉门控、只保留线性组合的简化版本演示同样的机制。假设 $\mathbf{h}_{t-1} = [0, 0]$（序列开头，隐状态清零），$z_{t-1} = 0.50$，$a_{t-1} = 1.0$，确定性更新用权重 $w_z = [0.4, 0.2]$、$w_a = [0.3, 0.1]$、偏置 $b = [0.1, 0]$：

$$
\mathbf{h}_t = w_z \cdot z_{t-1} + w_a \cdot a_{t-1} + b = [0.4, 0.2] \times 0.50 + [0.3, 0.1] \times 1.0 + [0.1, 0] = [0.60,\ 0.20]
$$

再用先验网络（同样简化为线性层）$w_\mu = [0.5, 0.5]$、固定 $\sigma_{\text{pr}} = 0.20$，算出先验分布的均值：

$$
\mu_{\text{pr}} = w_\mu \cdot \mathbf{h}_t = 0.5 \times 0.60 + 0.5 \times 0.20 = 0.40
$$

最后用重参数化技巧采样 $z_t$（采样噪声 $\varepsilon = 0.50$）：

$$
z_t = \mu_{\text{pr}} + \sigma_{\text{pr}} \cdot \varepsilon = 0.40 + 0.20 \times 0.50 = 0.50
$$

这条链路展示了 RSSM 单步转移的完整数据流：$(\mathbf{h}_{t-1}, z_{t-1}, a_{t-1}) \to \mathbf{h}_t \to \mu_{\text{pr}} \to z_t$。真实代码里 $f_\phi$ 是 PyTorch 的 `GRUCell`（带重置门和更新门，见前面的 GRU 深挖框），先验网络也不止一层线性层，算出来的具体数值会不同，但数据流经过的每一步和这里完全一致。P02 项目会用同样的数字跑一遍这个简化链路，验证公式和代码对得上。
...
接下来阅读[训练分布与自由 Rollout](./03-training-and-rollout)，检查这些模型在连续使用自身预测时会发生什么。完成这一步后再进入 P02，单步误差与长时程漂移的区别会更加具体。
