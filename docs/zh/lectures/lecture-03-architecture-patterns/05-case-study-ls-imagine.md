...
> **📖 MineCLIP**（[Fan et al., 2022](https://arxiv.org/abs/2206.08853)）：一个 CLIP 风格的模型（CLIP 见 L01），专门在 Minecraft 游戏视频和对应解说文本上预训练，因此能够评估一段短视频片段与一句自然语言任务描述之间的匹配程度。这里把它复用为奖励信号：不是用来做图像分类，而是用来判断模拟出的探索轨迹是否正在朝文本描述的目标靠近。
...
> **📖 本次对比中的基线方法**：**VPT**（Video PreTraining，[Baker et al., 2022](https://arxiv.org/abs/2206.11795)）先用少量带标注的数据训练一个从视频推断动作的逆动力学模型，再用它给数百万小时无标注的 Minecraft 游戏视频打上伪动作标签，用于行为克隆式的策略预训练。**STEVE-1**（[Lifshitz et al., 2023](https://arxiv.org/abs/2306.00937)）在 VPT 的基础上，借鉴语言模型指令微调的思路，训练出一个可以按文本或视觉目标条件化的策略。**PTGM**（Pretraining with Task-Guided Merging，[Yuan et al., 2024](https://arxiv.org/abs/2405.14684)）把 VPT 的动作空间离散化成一个较小的技能码本，使高层任务指令可以映射到可复用的短动作序列。**Director**（[Hafner et al., 2022](https://arxiv.org/abs/2206.04114)）是 Dreamer 的层次化扩展：一个管理者（manager）策略在潜空间中设定子目标，一个执行者（worker）策略学习去达成这些子目标，针对的正是 LS-Imagine 用跳跃式状态转换来解决的同一个长期“短视”问题。以上四种方法在本文中都是 LS-Imagine 所对比的无模型或有模型基线。
