...
**"如果一个表示不能被可视化或解码回像素，就没办法判断它好不好。"** L02 给出过检验表示的三类证据：重建、探针、下游预测。JEPA 在设计上放弃了第一类，但另外两类依然适用：探针可以检查物体位置或接触状态能否从 $s_x$ 中线性读出，下游任务表现可以衡量这个表示是否真的有用。非生成式不等于无法评估，只是重建这一列证据不可用，另外两列必须承担起全部的举证责任。

**"长时程 rollout 漂移是数据问题，用更多真实交互数据训练就能解决。"** Self-Forcing 要解决的差距不是数据量不足，而是训练时模型看到的（teacher forcing 下的真实状态）和推理时模型必须消费的（自己的预测）之间的不匹配。用同样的训练流程喂入更多同类数据，只会扩大模型见过真实状态的范围，却从来不会让模型接触到自己会累积的误差。Self-Forcing 是通过改变训练时喂给模型什么，偶尔喂入它自己此前的预测，来缩小这个差距，而不是靠增加更多同类数据。
