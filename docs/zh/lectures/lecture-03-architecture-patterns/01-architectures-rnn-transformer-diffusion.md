...
> **📖 softmax 与自注意力中的 Q、K、V**：softmax 将任意实数向量 $[x_1, x_2, \ldots, x_n]$ 转换为概率分布（所有元素非负且之和为 1）：$\text{softmax}(x_i) = \frac{e^{x_i}}{\sum_j e^{x_j}}$。较大的 $x_i$ 对应较大的输出概率，较小的 $x_i$ 对应接近零的概率。自注意力先把每个位置的向量线性变换为三个角色：**Query**（查询，Q）表示当前位置想"问什么"，**Key**（键，K）表示其他位置"提供什么信息"，**Value**（值，V）则携带实际信息。$QK^\top$ 计算每对位置的相关性得分，除以 $\sqrt{d_k}$ 以避免点积随维度增大而导致 softmax 过于尖锐，再由 softmax 将得分归一化为权重，最后对 $V$ 加权求和。
