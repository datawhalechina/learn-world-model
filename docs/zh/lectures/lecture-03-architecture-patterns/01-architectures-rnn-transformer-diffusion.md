...
**"架构越新就越好，选最新的准没错。"** Transformer 和扩散骨干在某些基准上分数更高，但都有代价：Transformer 的计算量随序列长度呈平方增长，需要更多数据才能收敛；扩散采样每帧需要 10 到 100 步去噪，且端到端不可微分。只要在线延迟或者需要对动力学模型直接求梯度比原始序列长度或视觉保真度更重要，RSSM 仍然是正确的选择，这正是 L03 规划部分 Dreamer 的 Actor-Critic 所处的情形。

**"更长的上下文窗口是对 RSSM 有限记忆的免费升级。"** 自注意力确实去掉了 GRU 的隐状态瓶颈，让每个位置都能直接看到任意更早的时刻。但它没有去掉代价：注意力的计算量是序列长度的 $O(T^2)$。STORM 相对 IRIS 的改进，把每帧压缩成一个 token 而不是十六个，正是因为这个代价是真实存在、值得削减的。更长的上下文窗口是在计算量和数据量上做的权衡，而不是一种绝对占优的替代方案。

**"生成的帧看起来越好，世界模型就越好。"** Diamond 生成的帧在视觉保真度上超过此前所有世界模型，但视觉质量和可用的状态是两个不同的维度。扩散模型对每一帧几乎独立地去噪，没有显式的物体记忆，所以即便单帧看起来很有说服力，物体的身份、位置和遮挡关系仍可能在长序列中悄悄漂移。这是物体持续性问题，属于状态追踪上的失败，更好的图像质量并不能修复它。

这三种误区都把某一个基准维度，新旧、上下文长度或视觉保真度，当成了单独就能决定架构选择的东西。真正决定选择的是你的部署环路里哪个约束在起作用：延迟、可微分性、计算预算，还是长时程的身份追踪。
