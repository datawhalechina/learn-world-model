---
title: "选读前沿：LoopWM、WAM 与架构选型"
description: LoopWM 的循环动力学与谱稳定化、WAM 联合学习第四范式、九大架构族对比总结与选型建议。
lecture: 3
---

# 选读前沿：LoopWM、WAM 与架构选型
...
Genie 证明了"从视频隐式发现动作表征"这条路可行。WAM 系列接过这个思路，进一步追问：世界模型和策略模型，真的需要是两个分开的模块吗？设想一只机械臂，在动手之前先被逐帧展示一段自己成功抓起水杯的视频：WAM 想问的是，一个被训练来预测这段视频的模型，能不能在同一次前向传播里，顺带输出让这一切发生的电机指令？
...
传统的 World Model 以动作为输入、预测未来状态，是 policy 旁边的一个 simulator。VLA 绕过了世界模型，直接从视觉和语言指令预测动作，是一个端到端的 reactive policy。WAM 试图同时做两件事：预测世界的未来状态，同时预测应该采取的动作。世界的视觉演化成为动作学习的 **dense supervision**（密集监督），而不只是一个辅助任务。这与稀疏监督相对：稀疏监督只在 episode 结束时给出一次奖励，而这里每一帧视频都提供一次梯度信号，学习信号因此更丰富、更频繁。

**[Motus](https://arxiv.org/abs/2512.13030)**（Bi et al., 2025）沿用了 Genie 的 LAM 模块里那套 **latent action** 思路，但把它推向了真实机器人控制，而不是可交互视频生成。异构视频数据（包括大量没有动作标签的人类视频和机器人演示）中，连续的 latent action 会被自动抽取出来；随后，少量带真实机器人动作标签的数据把这些 latent action 对齐到真实控制。Motus 的核心贡献是把"从无标注视频中发现 latent action"和"用少量对齐数据迁移到真实控制"两个步骤整合进一个统一框架，在灵巧操作和运动任务上验证了跨具身迁移能力。
...
| **空间 3D/4D** | 以观察型为主 | 显式多视角一致性、物体恒存性 | 渲染开销大、需要多视角或位姿数据 | 自动驾驶感知/预测、精确操作 |
