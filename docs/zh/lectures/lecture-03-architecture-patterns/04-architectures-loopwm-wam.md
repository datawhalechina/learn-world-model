...
**"更强的动力学模型需要更多参数。"** LoopWM 的整个前提就是深度和参数量可以分离：同一个参数共享的 Recurrent Block，在困难的转移上可以多循环几次，在简单的转移上少循环几次，用额外的计算量换取预测精度，而不是用额外的权重。迭代式的潜在深度是与模型规模、数据量并列的第三条扩展轴，而不是其中任何一条的变体。

**"世界模型和策略必然是两个独立的模块，这才是让一个系统在架构上算作世界模型的原因。"** L01 的契约是用接口定义世界模型的：状态估计、转移、发射、预测，而不是这些接口由多少个模块实现、由谁实现。WAM 是对这一点最直接的证明：同一次前向传播，既预测未来帧，也输出电机指令，中间根本没有一个独立的策略网络。这些接口是活在一个模块里还是分散在几个模块里，是工程选择，不是定义的一部分。
