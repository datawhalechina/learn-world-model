...
这一节的标题容易带来一个自然但错误的联想：既然动作是从视频中发现的，那它就可以像游戏手柄的输入一样直接接到机器人上。事实并非如此：latent action 只是一段编码，描述两帧之间发生了哪种变化，完全由像素差异推导得出，不保证与任何具体的电机指令或物理单位对齐。本节末尾的局限性说明直接点出了这个后果：要让 latent action 对真实控制器有意义，仍然需要一个额外的对齐步骤。
