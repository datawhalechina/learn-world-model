...
在介绍三种规划机制之前，还需要先补上一种世界模型架构：MuZero。之所以把它放在这里而不是 Part A，是因为它最核心的特征和它的规划方式密不可分：它的世界模型是专门为树搜索量身定制的，理解其中一个就必须理解另一个。阅读时请记住，MuZero 首先是一种独特的架构（一个完全没有像素级语义的世界模型），其次才是一种规划方法（MCTS），而且这种规划方法正是为了利用这种架构的形状而设计的。
