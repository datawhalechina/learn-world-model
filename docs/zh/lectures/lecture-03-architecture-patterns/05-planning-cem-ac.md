...
有了世界模型，智能体如何用它来选择动作？L02 的推球任务停在了这一步：RSSM 已经能预测某个候选动作会让小球发生什么。这一部分要问的是，这个预测怎样变成真正的那一推。它是 P03 的直接前置，也为后续架构比较提供规划语境。这里介绍三种规划机制：从最直觉的随机搜索，到 Dreamer 的想象训练，再到 TD-MPC 的混合方案。

在介绍三种规划机制之前，还需要先补上一种世界模型架构：MuZero。我们把它放在规划部分，是因为它最核心的特征与树搜索密不可分。它的世界模型就是为了支持搜索而设计的，如果只讲表示而不讲使用者，系统仍然是不完整的。

## 常见误区

在介绍三种机制本身之前，有三个关于"用世界模型做规划"意味着什么的假设值得先澄清。

**"规划就是像 MuZero 那样做树搜索。"** MCTS 只是使用预测价值的一种具体方式，和 MuZero 在离散动作上做树形搜索的设计绑定在一起。CEM-MPC 从不构建树：它并行采样整段动作序列，再围绕表现最好的样本重新拟合分布。Dreamer 的 Actor-Critic 在决策时根本不做搜索，策略网络早已通过在想象轨迹上反向传播训练好了，每个真实步只需直接输出一个动作。树搜索是 MuZero 这个具体设计的特性，不是使用世界模型预测的必要条件。

**"世界模型必须可微分才能用来规划。"** CEM-MPC 把模型当作黑箱，只需要它能向前滚动，从不对它求梯度。可微分性恰恰是 Dreamer 的 Actor-Critic 用来换取样本效率的特性，不是规划本身强加的前提。TD-MPC 把这一点分离得很清楚：它用可微分的一致性损失训练潜在动力学，但在决策时依然用不需要梯度的 CEM 搜索。

**"优化器越是用力压榨世界模型给出的奖励预测，找到的动作就越好。"** 这忽略了模型利用问题：激进的优化器同样可能找到那些"因为模型在那里犯了错"而得分高的动作，而不是因为这些动作真的好。这正是 CEM-MPC 每一步都重新规划、而不是照单执行一次搜索结果的原因，也是 Dreamer 限制想象步数并定期用真实数据重新训练的原因，还是 TD-MPC 用 TD 自举抑制累积模型误差的原因。三种机制都花了真功夫去提防模型自身的盲区，而不是更信任它。

这三种误区共享同一个根源：把某个具体系统的工程选择，MCTS、可微分反向传播，或不受约束的优化，误当成了用世界模型规划的普遍属性。真正因机制而异的问题要窄得多：优化器是否需要梯度，是否需要搜索树，以及如何防止自己利用模型本身的误差。
...
> **📖 CEM 与 MPC**：**CEM**（Cross-Entropy Method，交叉熵方法）是一种基于采样的优化算法：先从一个分布（如高斯）中采样大量候选解，保留目标值最优的一小部分精英样本，再用它们重新拟合分布，反复迭代，使采样逐渐集中到高质量区域。这里用 CEM 在动作序列空间中搜索最优动作，因此称为"射击法"（shooting method）。**MPC**（Model Predictive Control，模型预测控制）则规定如何使用搜索结果：每个时间步都用模型预测未来 $H$ 步，选出最优动作序列，但只执行第一个动作，下一步再重新规划。即使模型不完美，频繁重规划也能及时纠正误差。
...
**手算一遍**：假设某一轮精化里采样了 5 条候选动作序列，展开想象后算出的累计奖励分别是 $R = [3.0,\ 7.0,\ 1.0,\ 9.0,\ 4.0]$，取 Top-K（$K=2$）：按 $R$ 降序排列后保留前两名，也就是第 4 条（$R=9.0$）和第 2 条（$R=7.0$）。用这两条精英序列重新拟合分布：

$$
\mu_{\text{new}} = \frac{9.0 + 7.0}{2} = 8.0, \qquad \sigma_{\text{new}} = \sqrt{\frac{(9.0-8.0)^2 + (7.0-8.0)^2}{2}} = \sqrt{1.0} = 1.0
$$

下一轮采样就会以 $\mu=8.0, \sigma=1.0$ 为中心重新采样，比这一轮更集中在高回报区域，这就是 CEM 让搜索逐轮收缩的完整机制。
