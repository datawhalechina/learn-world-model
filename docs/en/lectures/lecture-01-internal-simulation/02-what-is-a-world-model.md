...
VLA (Vision-Language-Action) models are planners. CEM-MPC and TD-MPC are planners. The latent Actor-Critic inside Dreamer is a planner. The planner is the most compelling and the least mature of the three. The impressive-looking robot demonstrations of recent years are almost uniformly confined to tightly controlled laboratory settings; the gap between a demo video and a robot that reliably works in a real kitchen, warehouse, or operating room remains large.
...
World Labs' Marble already outputs Gaussian splats (for rendering) and collision meshes (for simulation) from a single model, dissolving the boundary between renderer and simulator. Work from several robotics labs has shown that pretrained video renderers can serve as backbones for joint world and action prediction, bridging renderer and planner.

The logical endpoint is a unified world model: a foundation model that renders photorealistic views, produces physically accurate structure, plans action sequences, and switches fluidly between output modalities depending on what the downstream consumer needs.
