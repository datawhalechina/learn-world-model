...
A natural but mistaken reading of this section's title is that once actions are discovered from video, they can be plugged into a robot as if they were joystick inputs. They cannot: a latent action is a code describing what kind of change occurred between two frames, induced entirely from pixel differences, with no guarantee it lines up with any particular motor command or physical unit. The Limitations note at the end of this section states the consequence directly, an alignment step is still required before a latent action means anything to a real controller.
