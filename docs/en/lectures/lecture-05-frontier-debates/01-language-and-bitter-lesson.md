...
This same limitation is also the source of the LLM's success. Different text tasks look wildly different on the surface (novels, papers, code, chat, math problems), but the mechanism that generates them is highly shared: the complex physical world is first understood and compressed by the human brain, then re-expressed as language. That shared "world to brain to language to token" pathway is what lets an LLM inherit thousands of years of accumulated human knowledge just by learning in Y space, arguably one of the deepest reasons foundation models succeed at all. But the same pathway also sets its ceiling: whatever information the brain discards while compressing the world (torque, material deformation, friction coefficients, and the like) never makes it into language, so the LLM can never learn it either. The brain is both the reason it wins and the reason it loses.
