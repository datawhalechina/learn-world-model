...
**"표현을 시각화하거나 픽셀로 다시 디코딩할 수 없다면, 그것이 좋은 표현인지 알 방법이 없다."** L02는 표현을 검증하는 세 가지 증거, 재구성, 프로브, 다운스트림 예측을 다룬 바 있습니다. JEPA는 설계상 첫 번째를 포기하지만, 나머지 둘은 그대로 적용됩니다. 프로브는 물체 위치나 접촉 상태가 $s_x$에서 선형적으로 읽어낼 수 있는지 확인할 수 있고, 다운스트림 과제 성능은 그 표현이 실제로 도움이 되는지를 측정합니다. 비생성형이라는 것이 평가 불가능을 뜻하지는 않습니다. 단지 그 증거표에서 재구성이라는 열이 비어 있을 뿐이고, 나머지 두 열이 그 무게를 짊어져야 한다는 뜻입니다.

**"장기 롤아웃 표류는 데이터 문제이므로, 실제 상호작용 데이터를 더 많이 학습시키면 사라진다."** Self-Forcing이 목표로 삼는 간극은 데이터 부족이 아닙니다. 학습 중 모델이 보는 것(teacher forcing 아래의 실제 상태)과 추론 중 모델이 소비해야 하는 것(자기 자신의 예측) 사이의 불일치입니다. 같은 학습 절차로 같은 종류의 데이터를 더 많이 흘려보내는 것은 실제 상태에 대한 노출만 넓힐 뿐, 모델이 스스로 누적시키는 오차에는 결코 노출시키지 못합니다. Self-Forcing은 모델이 학습하는 대상 자체를 바꿔서, 가끔은 자기 자신의 이전 예측을 학습 입력으로 사용해서 이 간극을 좁히는 것이지, 같은 종류의 데이터를 더 많이 넣어서가 아닙니다.
